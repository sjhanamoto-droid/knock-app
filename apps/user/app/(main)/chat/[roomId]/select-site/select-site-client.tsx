...
interface Props {
  initialSites: SiteList;
  roomId: string;
}

export function SelectSiteClient({ initialSites, roomId }: Props) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const companyId = searchParams.get("companyId");

  const [sites, setSites] = useState<SiteList>(initialSites);
