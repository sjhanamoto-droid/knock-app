...
interface Props {
  ...
  initialIsOrderer: boolean;
}

export function ChatOrdersClient({ initialOrders, initialIsOrderer }: Props) {
  const router = useRouter();
  const searchParams = useSearchParams();

  const [orders] = useState<OrderList>(initialOrders);
  const [search, setSearch] = useState("");
  const [isOrderer] = useState(initialIsOrderer);
