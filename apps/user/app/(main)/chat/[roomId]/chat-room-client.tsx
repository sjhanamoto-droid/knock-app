...
export function ChatRoomClient({ initialData, roomId }: Props) {
              ...
              {showMenu && (
                <>
                  ...
                  <div className="absolute right-0 top-full z-50 mt-1 w-48 rounded-xl border border-gray-200 bg-white py-1 shadow-lg">
                    ...
                    {isNegotiation && (
                      <button
                        onClick={() => { router.push(`/chat/${roomId}/orders?companyId=${partnerCompanyId}`); setShowMenu(false); }}
                        className="flex w-full items-center gap-2.5 px-4 py-2.5 text-left text-[14px] text-knock-text active:bg-gray-50"
                      >
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    ...
                    {/* 現場ルーム: 工事完了（現場全体） */}
                    {isSiteInfo && confirmedOrders.length > 0 && data.room.factoryFloor && (
                      <button
                        onClick={() => { router.push(`/work-completion/${data.room.factoryFloor!.id}`); setShowMenu(false); }}
                        className="flex w-full items-center gap-2.5 px-4 py-2.5 text-left text-[14px] text-knock-text active:bg-gray-50"
                      >
                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                          <path d="M2 8.5L6 12.5L14 3.5" stroke="#666" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
                        </svg>
                        工事完了
                      </button>
                    )}
                    {/* 現場ルーム: 発注者 追加工事（発注済み・施工中・工事完了。完了後の追加工事で現場は施工中に戻る） */}
                    {isSiteInfo && isOrderer && ["ORDERED", "IN_PROGRESS", "COMPLETED"].includes(data.room.factoryFloor?.status ?? "") && (
                      <button
