...
const sections: Section[] = [
  ...
  },
  {
    ...
    when:
      "あなたが受注した後、発注者が内容を確定すると注文書が発行され通知が届きます。注文書一覧（メニューの「帳票」／「注文書」）から該当の注文書をタップすると下の画面が開きます。",
    points: [
      "注文書番号（No. ORD-…）／発行日",
  ...
  },
  {
    num: 5,
    chip: "施工報告 → 工事完了",
    title: "施工が完了したら（施工報告＝工事完了）",
    when: "施工が終わったら、発注書ごとに「施工報告」を送ります。送信した発注書はその場で完了となり、完了した月の請求対象になります。",
    steps: [
      {
        title: "施工報告を送る（発注書ごと）",
        desc: "各発注書について施工報告を送ります。通知、または工事完了画面の発注書カードの「施工報告を提出して完了する」から開きます。",
        points: [
          "施工完了日は送信した日で自動的に確定します（選択はできません）",
          "施工報告コメント（任意・「✨AIで下書き」で文案を生成できます）",
          "施工写真（任意・「+ 追加」から複数添付可）",
          "「施工報告を送信して完了する」→ 確認（この発注書は工事完了となり、完了月の請求対象になります）で送信。送信後の変更はできません",
          "誤って送信した場合は発注者に差し戻しを依頼してください。差し戻されると通知が届き、内容を修正して再送信すると再び完了になります",
        ],
        actions: [{ label: "施工報告を送信して完了する", kind: "primary" }],
        images: [{ src: IMG("05-completion-report.png"), cap: "施工報告画面" }],
      },
      {
        title: "工事完了画面で進み具合を確認",
        desc: "発注書（本工事「発注書 1」・追加工事）の一覧と、それぞれの完了状況を確認できます。",
        points: [
          "各カードに「完了」「施工報告 未提出」「施工報告 再送信待ち」「回答待ち」の状態と金額を表示",
          "各発注書のカードをタップすると、その依頼の明細（証左）を展開して確認できます",
          "すべての発注書の施工報告を送ると、現場は「工事完了」になります",
          "工事完了後に発注者から追加工事の依頼が届くと、現場は「施工中」に戻ります。追加分の施工報告を送ると再び「工事完了」になります",
        ],
        images: [{ src: IMG("06-work-completion.png"), cap: "工事完了画面" }],
      },
    ],
    next:
      "完了した発注書の注文書金額は、完了した月（発注者の締め日基準）の請求対象になります。発注者の承認や「工事を締める」操作は不要です。",
  },
];
        ...
        <section className="rounded-2xl bg-white p-5 shadow-[0_1px_8px_rgba(0,0,0,0.06)]">
          ...
          <ol className="flex flex-col gap-3">
            {[
              ...
              "発注者が確定 → 注文書を確認",
              "（必要に応じて）追加工事を承諾",
              "施工 → 施工報告を送信（発注書ごと・送信で完了）",
              "すべての発注書が完了 → 工事完了 → 請求書発行へ",
            ].map((t, i) => (
