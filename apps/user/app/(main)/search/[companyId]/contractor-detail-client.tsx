...
interface Props {
  initialContractor: ContractorData | null;
  companyId: string;
}

export function ContractorDetailClient({ initialContractor, companyId }: Props) {
  const router = useRouter();
  const { toast } = useToast();

  const [contractor] = useState<ContractorData | null>(initialContractor);
  const [sending, setSending] = useState(false);
  const [processing, setProcessing] = useState(false);
  const [showConfirm, setShowConfirm] = useState(false);
  const [showApproveConfirm, setShowApproveConfirm] = useState(false);
  const [showRejectConfirm, setShowRejectConfirm] = useState(false);
  const [invitedId, setInvitedId] = useState<string | null>(initialContractor?.invitedId ?? null);
  const [inviteStatus, setInviteStatus] = useState<
    "none" | "sent" | "received" | "connected"
  >(initialContractor?.invitationStatus ?? "none");
