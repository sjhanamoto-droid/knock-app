...
export function OrderConfirmClient({ siteId, companyId, initialSite, initialContractor }: Props) {
  const router = useRouter();
  const { accentColor } = useMode();

  const [site] = useState<SiteData>(initialSite);
  const [contractor] = useState<ContractorData>(initialContractor);
  const [message, setMessage] = useState("");
  const [submitting, setSubmitting] = useState(false);
  const [showConfirm, setShowConfirm] = useState(false);
  const [error, setError] = useState("");
  const [sendingRequest, setSendingRequest] = useState(false);
  const [localInvitationStatus, setLocalInvitationStatus] = useState<"none" | "sent" | "received" | "connected">(
    initialContractor.invitationStatus
  );
  const [successMessage, setSuccessMessage] = useState("");
  const [redirectPath, setRedirectPath] = useState("");
  const [submitted, setSubmitted] = useState(initialSite.status !== "NOT_ORDERED");
