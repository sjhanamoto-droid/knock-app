...
type Props = {
  siteId: string;
  initialSite: SiteDetail;
  initialProjectSummary: ProjectSummary | null;
};

export function SiteDetailClient({ siteId, initialSite, initialProjectSummary }: Props) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { toast } = useToast();
  const { isOrderer, accentColor } = useMode();
  const [site, setSite] = useState<SiteDetail>(initialSite);
