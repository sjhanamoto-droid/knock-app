...
import type { getProfile } from "@/lib/actions/profile";
...
export function MyPageClient({ profile, trustScoreValue }: MyPageClientProps) {
        ...
        <Link href="/mypage/trust-score" className="shrink-0 flex flex-col items-center">
          ...
          >
            <span className="text-[13px] font-bold" style={{ color: accentColor }}>
              {trustScoreValue > 0 ? trustScoreValue.toFixed(1) : "-.-"}
            </span>
          </div>
      ...
      <div className="flex flex-col gap-3 px-4 pb-8">
        {activeTab === "personal" && (
          <SectionCard title="基本情報" editHref="/mypage/edit">
            ...
            <FieldRow label="性別" value={profile.gender ? genderLabels[profile.gender] : null} />
            <FieldRow
              label="生年月日"
              value={
                profile.dateOfBirth
            ...
            <FieldRow
              ...
              value={
                profile.dateOfBirth
                  ? `${Math.floor((Date.now() - new Date(profile.dateOfBirth).getTime()) / (1000 * 60 * 60 * 24 * 365.25))}歳`
                  : null
              }
            />
            ...
            <FieldRow label="就労資格" value={profile.workEligibility ? workEligibilityLabels[profile.workEligibility] : null} />
            <FieldRow
              label="保有資格"
              value={
        ...
        {activeTab === "business" && (
          <>
            {profile.company ? (
              <SectionCard title="会社情報" editHref="/mypage/company?section=info">
                <FieldRow label="企業名" value={profile.company.name || null} />
                <FieldRow label="種別" value={companyTypeLabels[profile.company.type] ?? profile.company.type} />
                ...
                <FieldRow
                  ...
                  value={
                    profile.company.prefecture || profile.company.city
                      ? [profile.company.prefecture, profile.company.city, profile.company.streetAddress, profile.company.building].filter(Boolean).join(" ")
                      : null
                  }
                />
                <div className="flex flex-col gap-0.5 py-3.5 border-b border-gray-100 last:border-b-0">
                  ...
                  {profile.company.stampImage ? (
                    <img src={profile.company.stampImage} alt="印鑑" className="mt-1 h-16 w-16 rounded border border-gray-200 object-contain bg-white p-1" />
                  ) : (
            ...
            <SectionCard title="受発注情報" editHref="/mypage/company?section=order">
              <FieldRow label="事業形態" value={profile.company?.companyForm ? companyFormLabels[profile.company.companyForm] : null} />
              <FieldRow label="対応エリア" value={profile.company?.areas && profile.company.areas.length > 0 ? profile.company.areas.map((a) => a.area.name).join(", ") : null} />
              <FieldRow label="稼働可能人数" value={profile.company?.workforceCapacity ? workforceCapacityLabels[profile.company.workforceCapacity] : null} />
              <FieldRow label="対応職種" value={profile.company?.occupations && profile.company.occupations.length > 0 ? profile.company.occupations.map((o) => o.occupationSubItem.name).join(", ") : null} />
            </SectionCard>
            ...
            <SectionCard title="振込先口座" editHref="/mypage/company?section=bank">
              <FieldRow label="銀行名" value={profile.company?.bankName || null} />
              <FieldRow label="支店名" value={profile.company?.bankBranchName || null} />
              <FieldRow label="口座種別" value={profile.company?.bankAccountType ? bankAccountTypeLabels[profile.company.bankAccountType] ?? profile.company.bankAccountType : null} />
            ...
            {!isContractor && (
              <SectionCard title="請求設定" editHref="/mypage/company?section=billing">
                <FieldRow label="締め日" value={profile.company?.billingClosingDay ? `毎月${profile.company.billingClosingDay}日` : profile.company?.paymentDueType ? "月末" : null} />
                <FieldRow label="猶予日数" value={profile.company?.billingGraceDays != null ? `${profile.company.billingGraceDays}日` : null} />
                <FieldRow label="支払期日" value={profile.company?.paymentDueType ? paymentDueTypeLabels[profile.company.paymentDueType] : null} />
              </SectionCard>
            )}
          </>
        )}
