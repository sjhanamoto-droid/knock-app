...
type MajorItem = { id: string; name: string; subItems: { id: string; name: string }[] };
...
interface Props {
  ...
  initialMasters: MajorItem[];
  initialSelections: Selection[];
  initialAreaMasters: AreaMaster[];
}

export function CompanyEditClient({ initialProfile, initialMasters, initialSelections, initialAreaMasters }: Props) {
  ...
  // Stamp image state
  const [stampPreview, setStampPreview] = useState<string | null>(c?.stampImage ?? null);
  const [uploadingStamp, setUploadingStamp] = useState(false);

  // Occupation state
  const [masters] = useState<MajorItem[]>(initialMasters);
  ...
  // 受発注情報 state
  const [areaMasters] = useState<AreaMaster[]>(initialAreaMasters);
  const [selectedAreaIds, setSelectedAreaIds] = useState<string[]>(c?.areas?.map((a) => a.area.id) ?? []);
  const [companyForm, setCompanyForm] = useState(c?.companyForm ?? "");
  const [workforceCapacity, setWorkforceCapacity] = useState(c?.workforceCapacity ?? "");
  ...
  const [selectedInsurances, setSelectedInsurances] = useState<string[]>(c?.insurances?.map((i) => i.type) ?? []);
  const [savingLicense, setSavingLicense] = useState(false);
  const [licenseSaved, setLicenseSaved] = useState(false);

  // 請求設定 state
  const [billingClosingDay, setBillingClosingDay] = useState(c?.billingClosingDay?.toString() ?? "");
  const [billingGraceDays, setBillingGraceDays] = useState(c?.billingGraceDays?.toString() ?? "5");
  const [paymentDueType, setPaymentDueType] = useState(c?.paymentDueType ?? "");
  const [savingBilling, setSavingBilling] = useState(false);
  const [billingSaved, setBillingSaved] = useState(false);

  if (!initialProfile?.company) {
