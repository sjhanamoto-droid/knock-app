...
interface Props {
  initialCheck: RequirementCheck;
}

export function RequirementsClient({ initialCheck }: Props) {
  const router = useRouter();
  const searchParams = useSearchParams();
  const returnTo = searchParams.get("returnTo") || "/mypage";

  const [saving, setSaving] = useState(false);
  const [check] = useState<RequirementCheck>(initialCheck);

  const d = initialCheck.data;
  // Form state
  const [workEligibility, setWorkEligibility] = useState(d.workEligibility ?? "");
  ...
  const [bankBranchName, setBankBranchName] = useState(d.bankBranchName ?? "");
  const [bankAccountType, setBankAccountType] = useState(d.bankAccountType ?? "");
  const [bankAccountNumber, setBankAccountNumber] = useState(d.bankAccountNumber ?? "");
  const [bankAccountName, setBankAccountName] = useState(d.bankAccountName ?? "");
  const [insuranceTypes, setInsuranceTypes] = useState<string[]>(d.insuranceTypes.length > 0 ? d.insuranceTypes : []);
