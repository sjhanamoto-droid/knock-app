...
type QualMaster = { id: string; name: string; category: string | null };

interface Props {
  initialProfile: Profile;
  initialQualMasters: QualMaster[];
}

export function EditProfileClient({ initialProfile, initialQualMasters }: Props) {
  ...
  const [formData, setFormData] = useState({
    ...
    lastNameKana: initialProfile?.lastNameKana ?? "",
    firstNameKana: initialProfile?.firstNameKana ?? "",
    email: initialProfile?.email ?? "",
    telNumber: initialProfile?.telNumber ?? "",
    dateOfBirth: initialProfile?.dateOfBirth ?? "",
    gender: (initialProfile?.gender ?? "") as string,
    workEligibility: (initialProfile?.workEligibility ?? "") as string,
    tradeName: initialProfile?.tradeName ?? "",
    workersCompInsurance:
      initialProfile?.workersCompInsurance === true
        ? "true"
        : initialProfile?.workersCompInsurance === false
        ? "false"
        : "",
  ...
  // Qualification state
  const [qualMasters] = useState<QualMaster[]>(initialQualMasters);
  const [selectedQualIds, setSelectedQualIds] = useState<string[]>(
    initialProfile?.qualifications?.map((q) => q.qualification.id) ?? []
  );
