...
type SlotData = { date: string; status: string };
...
export function AvailabilityClient({ initialSlots, initialIsPublic }: Props) {
  ...
  const buildInitialSlots = (): Record<string, SlotStatus> => {
  ...
  const loadMonthData = useCallback(async (ym: string) => {
    setLoading(true);
    try {
      const slotsData = await getAvailabilitySlots(ym);
      const map: Record<string, SlotStatus> = {};
